...
基于之前群内猫大分享的攻防思想初步制作.模拟当判定服务器无响应时,客户端自动切换路由地址.目前是固定连接路由1.掉线就切换路由2.演示用

## 使用方法

开启三个进程.分别对应进程id 1,2,3.  2和3是路由进程
当玩家进入游戏后,手动关闭router1,即进程id为2的进程.当客户端检测超时后会自动切换到router2.然后游戏继续玩
